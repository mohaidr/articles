...
- **Multi-threading (تعدد الخيوط):** تشغيل كودين في نفس اللحظة فعلياً عن طريق إنشاء خيوط (Threads) إضافية.
- **Async (الانتظار الذكي):** تحرير الـ Thread الحالي من الانتظار أثناء جلب البيانات، بدون إنشاء thread جديد.

### مثال مقارن كامل — مع استدعاء من Main
...
| المعيار | Multi-threading | Async/Await |
|---------|----------------|-------------|
| **Threads المستخدمة** | thread جديد لكل عملية | نفس الـ thread، يُحرَّر أثناء الانتظار |
| **استهلاك الذاكرة** | ~1MB stack لكل thread | لا شيء تقريباً |
| **1000 طلب شبكة متزامن** | 1000 thread = كارثة! | يُدار بعدد صغير من threads |
| **مناسب لـ** | عمليات CPU الثقيلة | شبكة، قاعدة بيانات، ملفات |
...
- **Concurrency:** التبديل السريع بين المهام (كالأم التي تهتم بشؤون عائلتها فتغسل و تطبخ و تجلي و ترعى الأطفال و تنتقل بين أعمالها و كأنها تنجزها جميعا معا و لكنها في الحقيقة تتنقل بينهم) قد يبدو .
- **Parallelism:** تنفيذ المهام معاً في نفس اللحظة (كأنك تطبخ وصديقك يغسل الأطباق).
...
|--|-------------|-------------|
| **ما يحدث فعلاً** | التنسيق بين مهام متداخلة زمنياً | تنفيذ حقيقي في نفس اللحظة |
| **Threads المستخدمة** | قد يكون thread واحد (مع async) | cores/threads متعددة |
| **مناسب لـ** | I/O: شبكة، ملفات، قاعدة بيانات | CPU: حسابات، معالجة صور، ضغط |
| **المثال** | `Task.WhenAll` لـ 3 API calls | `Parallel.For` لمعالجة بيانات |
...
| الأداة | متى تستخدمها |
|--------|--------------|
| `Parallel.ForEach` | عندما يكون لديك آلاف البيانات وتريد معالجتها بأقصى سرعة (Data Parallelism) |
